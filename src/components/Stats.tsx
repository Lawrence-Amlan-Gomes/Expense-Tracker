...
function shortMonth(name: string): string {
  return new Date(name + " 1").toLocaleString("default", {
    month: "short",
    year: "2-digit",
  });
}
...
function EmptyState({ theme }: { theme: boolean }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs opacity-30 hidden sm:block">
        Add spending entries to see charts
      </p>
    </div>
  );
}
...
export default function Stats() {
  ...
  useEffect(() => {
    if (hasMounted && auth === null) router.push("/login");
    ...
    () =>
      months.flatMap((m) =>
        m.spendings.map((s) => {
          ...
          const subItems = Object.entries(parsed)
            .filter(([k]) => k !== "Name")
  ...
  const availableYears = useMemo(() => {
    const years = new Set(
      months.map((m) => new Date(m.name + " 1").getFullYear()),
    );
  ...
  const monthsForYear = useMemo(
    () =>
      months
        .filter((m) => new Date(m.name + " 1").getFullYear() === selectedYear)
        .sort(
          (a, b) =>
            monthToDate(a.name).getTime() - monthToDate(b.name).getTime(),
        ),
    [months, selectedYear],
  );
  ...
  const categoryData = useMemo(() => {
    ...
    flatSpendings
      .filter(
        (f) => new Date(f.monthName + " 1").getFullYear() === selectedYear,
      )
      .forEach((f) => {
        f.subItems.forEach((si) =>
          map.set(si.name, (map.get(si.name) || 0) + si.cost),
        );
        if (f.subItems.length === 0)
          map.set(f.displayName, (map.get(f.displayName) || 0) + f.totalCost);
      });
    return [...map.entries()]
  ...
  const avgPerMonth = monthlyTotals.length
    ? Math.round(totalThisYear / monthlyTotals.length)
    : 0;
  ...
  const minMonth =
    minMonthCandidates.length > 0
      ? minMonthCandidates.reduce((a, b) => (b.total < a.total ? b : a))
      : null;

  const lastTwo = monthlyTotals.slice(-2);
  const momTrend: "up" | "down" | "neutral" =
    lastTwo.length === 2
      ? lastTwo[1].total > lastTwo[0].total
        ? "up"
        : lastTwo[1].total < lastTwo[0].total
          ? "down"
          : "neutral"
  ...
  const weeklyData = useMemo(() => {
    ...
    monthsForYear.forEach((m) => {
      const s = [...m.spendings].sort(
        (a, b) => parseInt(a.date) - parseInt(b.date),
      );
      const w1 = s
        .filter((x) => parseInt(x.date) <= 7)
        .reduce((sum, x) => sum + x.cost, 0);
      const w2 = s
        .filter((x) => parseInt(x.date) > 7 && parseInt(x.date) <= 14)
        .reduce((sum, x) => sum + x.cost, 0);
      const w3 = s
        .filter((x) => parseInt(x.date) > 14 && parseInt(x.date) <= 21)
        .reduce((sum, x) => sum + x.cost, 0);
      const w4 = s
        .filter((x) => parseInt(x.date) > 21)
        .reduce((sum, x) => sum + x.cost, 0);
  ...
  const radarData = useMemo(
    () =>
      monthlyTotals
        .slice(-6)
        .map((m) => ({ month: m.name, spending: m.total, avg: avgPerMonth })),
  ...
  const insights = useMemo(() => {
    ...
    if (monthlyTotals.length >= 2) {
      ...
      if (pct > 20)
        ins.push({
          type: "warning",
          title: `Spending Spike in ${last.name}`,
          body: `Your spending jumped ${pct}% compared to ${prev.name}. Review your largest categories to identify what drove this increase.`,
        });
      else if (pct < -15)
        ins.push({
          type: "success",
          title: `Great Savings in ${last.name}!`,
          body: `You reduced spending by ${Math.abs(pct)}% vs ${prev.name}. Keep this momentum going — consistency builds wealth.`,
        });
    }
    if (categoryData.length > 0) {
      const top = categoryData[0];
      const pct =
        totalThisYear > 0 ? Math.round((top.value / totalThisYear) * 100) : 0;
      if (pct > 30)
        ins.push({
          type: "warning",
          title: `"${top.name}" Dominates Your Budget`,
          body: `${pct}% of yearly spending goes to "${top.name}". Consider if this aligns with your financial priorities.`,
        });
    }
    if (avgPerMonth > 0)
      ins.push({
        ...
        body: `Your average monthly spend is ৳${avgPerMonth.toLocaleString()}. A 10% reduction target of ৳${Math.round(avgPerMonth * 0.9).toLocaleString()} could save ৳${Math.round(avgPerMonth * 0.1 * 12).toLocaleString()} per year.`,
      });
    if (weeklyData.length > 0) {
      const top = [...weeklyData].sort((a, b) => b.total - a.total)[0];
      ins.push({
        type: "tip",
        title: `Highest Spending: ${top.label}`,
        body: `You tend to spend most during ${top.label} (৳${top.total.toLocaleString()}). Planning purchases earlier in the month could improve budget control.`,
      });
    }
    ...
    if (ins.length === 0)
      ins.push({
        ...
        body: "Track at least 2–3 months of spending to unlock personalized financial insights and recommendations.",
      });
    return ins;
          ...
          <div className="space-y-4 sm:space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <StatCard
                ...
                theme={theme}
                trend="neutral"
                sub={`Across ${monthlyTotals.length} months`}
              />
              <StatCard
                ...
                theme={theme}
                trend={momTrend}
                sub="Based on tracked months"
              />
              <StatCard
                label="Highest Month"
                value={maxMonth.total > 0 ? maxMonth.name : "—"}
                icon={Flame}
                accent="#f43f5e"
                theme={theme}
                sub={
                  maxMonth.total > 0
                    ? `৳ ${maxMonth.total.toLocaleString()}`
                    : undefined
                }
              />
              <StatCard
                label="Lowest Month"
                value={minMonth ? minMonth.name : "—"}
                icon={Target}
                accent="#f59e0b"
                theme={theme}
                sub={
                  minMonth ? `৳ ${minMonth.total.toLocaleString()}` : undefined
                }
              />
            </div>
            ...
            >
              <SectionHeader
                ...
                sub="Total monthly spend across all tracked periods"
                theme={theme}
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              >
                <SectionHeader
                  ...
                  sub="How spending distributes across weeks"
                  theme={theme}
                />
                {weeklyData.length > 0 ? (
              ...
              >
                <SectionHeader
                  ...
                  sub="Which days you typically spend more"
                  theme={theme}
                />
          ...
          <div className="space-y-4 sm:space-y-8">
            ...
            >
              <SectionHeader
                ...
                sub="Total expenditure for each month"
                theme={theme}
              />
                    ...
                    <Bar dataKey="total" name="Spending" radius={[7, 7, 0, 0]}>
                      {monthlyTotals.map((m, i) => (
                        <Cell
                          key={i}
                          fill={
                            m.total === maxMonth.total ? "#f43f5e" : "#6366f1"
                          }
                          fillOpacity={0.85}
                        />
                      ))}
            ...
            >
              <SectionHeader
                ...
                sub="Spending each month vs. your average"
                theme={theme}
              />
              {monthlyTotals.length > 0 ? (
                <ResponsiveContainer width="100%" height={220}>
                ...
                <>
                  ...
                  >
                    {monthlyTotals.map((m, i) => {
                      ...
                      const share =
                        totalThisYear > 0
                          ? Math.round((m.total / totalThisYear) * 100)
                          : 0;
                      return (
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                              ...
                              <span
                                ...
                                style={{
                                  background:
                                    diff > 0 ? "#f43f5e22" : "#10b98122",
                                  color: diff > 0 ? "#f43f5e" : "#10b981",
                                }}
                              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
              ...
              >
                <SectionHeader
                  ...
                  sub={`Top categories by total spend in ${selectedYear}`}
                  theme={theme}
                />
              ...
              >
                <SectionHeader
                  ...
                  sub="Your top 10 spending categories"
                  theme={theme}
                />
                  ...
                  {categoryData.map((c, i) => {
                    const pct =
                      categoryData[0].value > 0
                        ? Math.round((c.value / categoryData[0].value) * 100)
                        : 0;
                    return (
                      <div key={i}>
          ...
          <div className="space-y-4 sm:space-y-6 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-6 sm:mt-8">
              {[
                ...
                {
                  value: months.reduce((s, m) => s + m.spendings.length, 0),
