...
type ViewTab = "overview" | "monthly" | "categories" | "insights";
 ...
 */
function EmptyState({ themeTokens }: { themeTokens: ThemeTokens }) {
  return (
    <div
 ...
 * // → [2025, 2024]
 */
function extractAvailableYears(months: IMonth[]): number[] {
  const yearSet = new Set<number>();

  for (const month of months) {
    const year = new Date(month.name + " 1").getFullYear();

    // Sanity check: only include reasonable years
...
export default function Stats() {
  ...
  const availableYears = useMemo(
    () => extractAvailableYears(allMonths),
    [allMonths]
  );
  ...
  const insights = useMemo(
    ...
    [completedMonthlyTotals, categoryData, totalThisYear, averagePerMonth, weeklyData]
  );
